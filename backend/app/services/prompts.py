...
class VastrAIPrompts:
    ...
    def __init__(self) -> None:
        self.garment_template = PromptTemplate.from_template(
            ...
            "{garment_style} {garment_type}. [image 1] is the fabric/garment "
        ...
        self.tryon_template = PromptTemplate.from_template(
            ...
            "that exactly matches the garment reference image. [image 1] is "
            "the person photo - keep their face, hair, pose, skin tone and "
            "body exactly as they are, and only replace their clothing. "
            "[image 2] is the garment reference image and is the "
            "{preservation_marker} - use only its garment for the clothes, and "
            "use the described fabric details only as supplementary hints "
            "({fabric_description}). Preserve Image 2's exact original colors, "
            "color combinations, patterns, prints, checks, stripes, textures, "
            "motifs, borders, and visible construction details. Do not "
            "reinterpret, redesign, recolor, simplify, or replace the garment "
            "design. If Image 2 contains a checkered or striped pattern, "
            "reproduce that pattern clearly and consistently instead of "
            "converting it into a solid color. The generated garment must "
            "visually match the Image 2 garment reference as closely as "
            "possible while naturally fitting the person's body. Change only "
            "the garment placement, scale, deformation, and lighting required "
            "to realistically fit the person - do not change the garment's "
            "original design or colors. Photorealistic, consistent lighting, "
